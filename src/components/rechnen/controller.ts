...
export class RechnenController extends AbstractController {
	...
	private updateTextAndColor(): void {
		this.ergebnisText =
			this.aufgabe.sign === '+' ? 'Addition' : this.aufgabe.sign === '-' ? 'Substraktion' : this.aufgabe.sign === '•' ? 'Multiplikation' : 'Division';
		this.ergebnisColor = this.aufgabe.sign === '+' ? '#009' : this.aufgabe.sign === '-' ? '#f80' : this.aufgabe.sign === '•' ? '#f0f' : '#0a7';
	}
	...
	public getRightWrongSum(results: AufgabeStore[]): {
		sum: number;
		right: number;
		wrong: number;
	} {
	...
	public getOrderedResults(): AufgabeStore[][][][] {
		...
		results.forEach((result: AufgabeStore) => {
			...
			orderedResults[date.getFullYear()][date.getMonth()][date.getDate()] = orderedResults[date.getFullYear()][date.getMonth()][date.getDate()] || [];
	...
	public getDayProgress(): {
		...
		orderedResults[date.getFullYear()] = orderedResults[date.getFullYear()] || [];
		orderedResults[date.getFullYear()][date.getMonth()] = orderedResults[date.getFullYear()][date.getMonth()] || [];
		const rightWrongSum = this.getRightWrongSum(orderedResults[date.getFullYear()][date.getMonth()][date.getDate()] || []);
